...
public class RCC_Camera : MonoBehaviour {
        ...
        switch (cameraSwitchCount) {
            ...
            case 1:
                if (useHoodCameraMode && cameraTarget.hoodCamera) {
                    GameObject.FindGameObjectWithTag("HideMesh").GetComponent<MeshRenderer>().enabled = false;
            ...
            case 2:
                GameObject.FindGameObjectWithTag("HideMesh").GetComponent<MeshRenderer>().enabled = true;
    ...
    private void TPS() {
        ...
        // Position at the target
        position = cameraTarget.playerVehicle.transform.position;
    ...
    public void Collision(Collision collision) {
        ...
        colRelVel = collision.relativeVelocity;
        colRelVel *= 1f - Mathf.Abs(Vector3.Dot(transform.up, collision.GetContact(0).normal));

        cos = Mathf.Abs(Vector3.Dot(collision.GetContact(0).normal, colRelVel.normalized));

        if (colRelVel.magnitude * cos >= 2f) {
    ...
    private void ResetCamera() {
        ...
        switch (cameraMode) {
            ...
            case CameraMode.FPS:
                ...
                actualCamera.transform.Rotate(15f, 0, 0, Space.Self);
            ...
            case CameraMode.WHEEL:
                ...
                transform.localPosition = Vector3.zero;
                actualCamera.transform.Rotate(0, 0, 0, Space.Self);
                transform.localRotation = Quaternion.identity;
    ...
    private void OccludeRay(Vector3 targetFollow) {
        if (Physics.Linecast(targetFollow, transform.position, out wallHit, occlusionLayerMask)) {
            ...
            if (!wallHit.collider.isTrigger && !wallHit.transform.IsChildOf(cameraTarget.playerVehicle.transform)) {
                ...
                occludedPosition.Set(wallHit.point.x + wallHit.normal.x * .2f, wallHit.point.y + wallHit.normal.y * .2f, wallHit.point.z + wallHit.normal.z * .2f);
                    //= new Vector3(wallHit.point.x + wallHit.normal.x * .2f, wallHit.point.y + wallHit.normal.y * .2f, wallHit.point.z + wallHit.normal.z * .2f);
    ...
    public IEnumerator AutoFocus() {
        ...
        timer = 3f;
        bounds = RCC_GetBounds.MaxBoundsExtent(cameraTarget.playerVehicle.transform);
    ...
    public IEnumerator AutoFocus(Transform transformBounds) {
        ...
        timer = 3f;
        bounds = RCC_GetBounds.MaxBoundsExtent(transformBounds);
    ...
    public IEnumerator AutoFocus(Transform transformBounds1, Transform transformBounds2) {
        ...
        timer = 3f;
        bounds = (RCC_GetBounds.MaxBoundsExtent(transformBounds1) + RCC_GetBounds.MaxBoundsExtent(transformBounds2));
    ...
    public IEnumerator AutoFocus(Transform transformBounds1, Transform transformBounds2, Transform transformBounds3) {
        ...
        timer = 3f;
        bounds = (RCC_GetBounds.MaxBoundsExtent(transformBounds1) + RCC_GetBounds.MaxBoundsExtent(transformBounds2) + RCC_GetBounds.MaxBoundsExtent(transformBounds3));
